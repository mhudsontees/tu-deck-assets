...
- Each slide's `<section>` class comes from that block's **Section tag** line (e.g. `class="slide type-stat-cards"`). Add ` active` to the **first** slide only.
...
```html
<section class="slide [SECTION-CLASS — see each block's Section tag line]" id="[semantic-id]">
...
> Each block shows the **exact `<section>` tag** to use — the class after `slide` names the block — and a **filled example** to copy and edit. Block 11 is the default for a normal heading + points slide.
...
> Modifiers are not slides — they are extras added **inside** an existing slide. New modifiers can be added here in future without renumbering the blocks above.
